...
class ThreeKingdomsWerewolfGame:
    """三国狼人杀游戏主类"""
    def __init__(self):
        self.players:Dict[str,ReActAgent] = {}#表示所有玩家的总表
        self.roles : Dict[str,str] = {}#表示“玩家名 -> 身份”的映射
        self.moderator = GameModerator()#游戏主持人
        self.alive_players : List[ReActAgent] = []#表示当前还活着的玩家列表
        self.werewolves : List[ReActAgent] = []#表示所有狼人玩家列表
        self.villagers : List[ReActAgent] = []#表示普通村民列表
        self.seer: List[ReActAgent] = []#表示预言家列表
        self.witch: List[ReActAgent] = []#表示女巫列表
        self.hunter: List[ReActAgent] = []#表示猎人列表
        
        # 女巫道具状态
        self.witch_has_antidote = True#表示女巫当前还有没有解药
        self.witch_has_poison = True#表示女巫当前还有没有毒药
    ...
    async def day_phase(self,round_num:int):
        ...
        async with MsgHub(
            ...
            # 统计投票
            votes = {}
            for i, voted_msg in enumerate(voted_msgs):
                # 检查vote_msg是否为None或metadata是否存在
                if voted_msg is not None and hasattr(voted_msg,'metadata') and voted_msg.metadata is not None:
                    votes[self.alive_players[i].name] = voted_msg.metadata.get("vote")
                else:
                    # 如果返回无效,默认弃票

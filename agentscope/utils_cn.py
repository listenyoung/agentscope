...
def analyze_speech_pattern(speech:str)->Dict[str,Any]:
    ...
    for word in confidence_words:
        analysic["confidence_keywords"] += speech.count(word)
    for word in doubt_words:
        analysic["doubt_keywords"] += speech.count(word)
    # 简单情感分析
    positive_words = ["好", "棒", "赞", "支持", "同意"]
    negative_words = ["坏", "差", "反对", "不行", "错误"]
    for word in positive_words:
        analysic["emotion_score"] += speech.count(word)
